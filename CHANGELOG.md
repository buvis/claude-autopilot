...
- **run-autopilot**: `cli/brief.py` renders `dev/local/autopilot/session-brief.md` from `state.json` (where the batch stands, the contract card verbatim, the files the next gate needs, what not to re-read), so a fresh session orients from one page instead of a hundred tool calls
