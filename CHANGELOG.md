...
- **run-autopilot**: in loop mode the review session hands off right after the rework design and task-add, so the fix tasks run in a fresh session on their own tier instead of in the review session that already holds the consolidation and three design dispatches (measured: 464K at the first fix's start, then a mid-task rotation)
...
- **review-work-completion**: consensus matching now treats `path (lines a-b)`, `path (lines 18-22, 423)`, `path#L12-L20` and `N/A (a.py:77, b.py:91)` citations as the file they name, so citations naming one file no longer fail the file gate, and merges a terse restatement of a verbose same-file finding (the shorter wording's containment in the longer, PRD 00198's review-00186 fixture), so the four agreements that review rendered as `[1/4]` now read `[2/4]`; the personas ask for repo-relative `path:line` citations, and the consolidator notes on stderr when a row's citations matched only after suffix stripping
...
- **run-autopilot**: refuse an `autonomous_decisions` entry the batch report cannot render at the moment it is appended, naming the key it lacks, instead of drawing a blank row in the batch report at the end of the run
- **fast-track**: run spec cards through the full review roster without the autopilot loop phases, worked in argument order, with the repo-wide suite and the `--push` push happening once after the last item
...
- **run-autopilot**: review sessions run on opus at xhigh in cycle 1 and at high on cycle 2 and later, with `_AUTOPILOT_EFFORT_REVIEW` forcing one effort for every cycle and `_AUTOPILOT_EFFORT_REVIEW_RERUN` setting the rerun value
...
- **run-autopilot**: `render report` says on stderr when the attempt ledger it was pointed at is missing or is not a readable file, instead of silently rendering an empty Implementor Mix
- **run-autopilot**: `render report` reads the attempt ledger once instead of twice, so a ledger that becomes unreadable between the two reads no longer drops every implementor row with nothing on stderr
- **run-autopilot**: an appended `autonomous_decisions` entry is judged even when it differs from an existing one only by the type of a value, so a `true` or `1.0` where the report expects a whole-number cycle can no longer reach the batch report unvalidated
...
- **run-autopilot**: the batch report's per-PRD `Cycles:` line falls back to the closing batch record's count instead of rendering `?` when the per-PRD reset has already cleared `state.cycle`
...
- **review-work-completion**: the review computes tautological-test checks
  and hands them to every reviewer as facts. `detect_tautological_tests.py`
  flags test shapes that cannot fail (constant or self-comparing asserts,
  `A or B` hedges, swallowed exceptions, `raises(Exception)`, no assertion),
  and `replay_tests_against_base.py` runs the diff's touched tests against
  the pre-change code in a throwaway worktree and reports the ones that still
...
- **use-codex**: the hook doctor reports unreadable canonical sources per
  target, including during dry runs, handles null bytes in `_common.py`
  import checks across supported Python versions, and describes canonical-only
  failures without blaming sibling imports.
...
- **use-sonnet**: the headless prompt-mode dispatch sends the prompt on the
  child's stdin instead of as a positional argv token. A `-f` prompt file's
  contents are delivered on stdin exactly as read (a trailing newline is
  already stripped by the pre-existing `$(cat ...)` prompt read), an
  option-looking prompt (e.g. `- [ ] item`) is never parsed as a flag, and a
  missing or empty prompt still fails before any dispatch.
