...
3. **Resume from a rotation scar (PRD 00202).** Read `git log -1 --format=%s`. When the subject ends in "wip - rotated mid-task" for this task, read the commit body and continue at the step it names instead of dispatching Tess. A `test(` subject for this task with no wip commit (tests committed, rotation before Devon) continues at step 2.9. On either resume, re-derive both anchors from this task's own commits — `git log --format='%H %s'`, walking back by task, not by subject shape — a `test(<scope>)` commit, a `chore(<scope>): wip - rotated mid-task` commit, or any step-5 commit of this task (its `<scope>` and the task-id reference step 5 asks for; a rotation mid-5.5-retry leaves `wip -> feat -> test` in the log) — stopping at the first commit that is not this task's: `<task_base_sha>` is the parent of the earliest such commit (replacing item 1's HEAD, which is the resume commit itself and would put the pre-rotation half of the task outside steps 5.6, 5.65 and 5.7), and `<test_commit_sha>` is the newest `test(` commit among them (unset when there is none). Any other subject starts at step 2.5 as usual.
...
**Prose gate — Devon skips prose-pin tasks.** When every test file Tess created or changed for this task is a prose pin (its basename ends in `_prose.py`), skip Devon, record `devon: skipped:prose` in the task's attempt entry, and proceed to step 2.95. A substring pin cannot see negation, inversion or past-tense narration, so every Devon round on such a task ends `round exhausted` at that ceiling (four of four on 2026-09-14/15, about 55 minutes of Opus time with no kept strengthening).
...
**Deterministic routing table.** Read the claimed task's tier (`state.tasks[i].model`) and qwen-eligibility flag (`state.tasks[i].qwen_eligible`), then cross-reference the "Gemini-first tasks" UI definition in **Implementor Selection** above. The planner flag encodes backend + Haiku/Sonnet + exactly one writable file + no public-contract edit; missing eligibility is false. Preserve the existing `ui` → `tier` → `contract` → `files` exclusion precedence.

**Runtime write-set reconciliation (before implementor selection).** Read the exact newline-delimited `FILE_PATHS` scratch file rendered for Ivan; ignore blank lines and count distinct paths. Tess-owned read-only tests are not in this list. After the `fable` override, zero paths fail closed to Claude at the task tier (no Codex interception, `preflight_outcome: null`, attempt `qwen_excluded_reason: "files"`). A test-only write set keeps the non-Qwen Claude path at its task tier; derive `is_test_only` from the existing step-2.7 classification using repo-relative paths, never absolute path ancestors, and pass it in the effective copy. For an otherwise Qwen-eligible backend Haiku/Sonnet task, two or more paths replace eligibility with false and exclusion with `files` **in a per-task effective copy only**; stamp the eventual attempt `qwen_excluded_reason: "files"`, `preflight_outcome: null`. Apply the table and all six Codex interception fences to that effective copy: two/three-file tasks can still take Codex, otherwise Claude at tier. Exactly one non-empty path may reach row 5. Do not rewrite planner metadata or unrelated state, and do not replace an existing UI, tier, or contract exclusion. This fence also applies under `legacy`.
...
| 7 | Backend, effective `qwen_eligible == false` (or absent) | Claude at the task's tier (e.g. a two-file `sonnet` task → Claude Sonnet, subject to Codex interception) | — |
...
**Before a Qwen dispatch**, read `references/qwen-integration.md` § Output guards and run its `check_qwen_output.py before` command. It requires a clean implementation slice and canonical Tess tests, and captures this dispatch's baseline. A failed preparation skips Qwen and uses Claude at tier without spending a Qwen attempt; retain the guard diagnostic in the task output and preserve the actual preflight verdict; do not let pre-existing task edits masquerade as Qwen output.
...
A **Success** from Qwen must first run `check_qwen_output.py after` per `references/qwen-integration.md` § Output guards, **before staging, committing, or running step 5.5**. Only exit 0 continues to step 5; exit 1 enters `references/gate-failure.md` § Qwen output rejection; exit 2 stops acceptance until the guard/canonical tests can be verified. After a non-success Qwen result (including timeout/lost result), first stop the helper and verify it is gone, then run the same guard with `--tests-only` before any fallback or partial-output acceptance; exit 1 takes mutation recovery, exit 2 stops, exit 0 resumes the original failure handler without a no-edit classification. Other successful implementors continue to step 5. For timeout, context exceeded, error, lost/hung result, or a codex dispatch that timed out or wrote nothing: **read `references/gate-failure.md` § Step 4 result table before acting on it.**
...
- `references/final-verification.md` - Step 7 suite commands, improvised-suite rule, regression loop, the review's queued verification checks and the recorded verification result (read before the phase's one full-suite run)
