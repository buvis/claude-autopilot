...
3. **Resume from a rotation scar (PRD 00202).** Read `git log -1 --format=%s`. When the subject ends in "wip - rotated mid-task" for this task, read the commit body and continue at the step it names instead of dispatching Tess. A `test(` subject for this task with no wip commit (tests committed, rotation before Devon) continues at step 2.9. On either resume, re-derive both anchors from this task's own commits — `git log --format='%H %s'`, walking back while the subject is a `test(<scope>)` or `chore(<scope>): wip - rotated mid-task` commit for this task: `<task_base_sha>` is the parent of the earliest such commit (replacing item 1's HEAD, which is the resume commit itself and would put the pre-rotation half of the task outside steps 5.6, 5.65 and 5.7), and `<test_commit_sha>` is the newest `test(` commit among them (unset when there is none). Any other subject starts at step 2.5 as usual.
