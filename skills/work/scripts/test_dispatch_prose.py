...
# --- PRD 00167: foreground command budgets and the separation rule ----------


def test_subagent_dispatch_carries_both_budget_rules() -> None:
    # A foreground Bash call is bounded by nothing else: the Watchdog covers
    # Agent dispatches and helper scripts only. Measured 2026-08-27, a combined
    # inspect-and-lint command stalled 17 min 48 s in an unattended loop. Both
    # rules live in one file so there is one place to change a budget.
    dispatch = (_SKILL_MD.parent / "references" / "subagent-dispatch.md").read_text()
    ...
    assert "300000" in dispatch, (
        "references/subagent-dispatch.md no longer states the lint and "
        "narrow-tests budget (300000 ms). Without a number the rule is a "
        "sentiment and every call site improvises its own deadline."
    )


def test_the_verification_call_sites_name_the_separation_rule() -> None:
    # The rule has to reach the two procedures that run commands, not just the
    # file that defines it — step 7's suite and step 5.5's narrow gate are where
    # a combined command would actually get built.
    final = (_SKILL_MD.parent / "references" / "final-verification.md").read_text()
...
def test_the_dispatch_prologue_forbids_a_combined_bash_call() -> None:
    # Implementors inherit the rule through the prologue and nowhere else, so
    # the sentence has to be IN the verbatim line the personas carry — the
    # section's prose about the line is not what reaches Ivan.
    start = _TEXT.index("## Dispatch prologue")
    end = _TEXT.index("## Passing values to render_prompt.py", start)
    prologue = _TEXT[start:end]

    assert "Never combine an inspection" in prologue, (
        f"{_SKILL_MD}: the dispatch prologue no longer forbids combining an "
        "inspection with a test or lint invocation in one Bash call. Ivan and "
        "Tess inherit their Bash rules from this line alone."
    )

    ivan = (_SKILL_MD.parents[2] / "agents" / "ivan.md").read_text()
    assert "Never combine an inspection" in ivan, (
        "agents/ivan.md no longer carries the separation rule the dispatch "
        "prologue declares verbatim. SKILL.md stating it changes nothing on "
        "its own: the persona file is what the implementor actually reads."
    )
