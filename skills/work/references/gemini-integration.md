...
How to invoke Gemini for task implementation via the `${CLAUDE_PLUGIN_ROOT}/skills/use-gemini/scripts/gemini-run.sh` helper, which wraps the native Gemini CLI. Always pass the prompt with `-f <file>`. With no `-m`, the CLI uses its default model.
