...
**The null-elapsed rule.** An `end` call whose window — the start row's `queued_at` through this call's `ended_at` — contains a `handoff` row's `at` reports `elapsed_s: null` instead of the usual difference, and prefixes `detail` with `spans handoff; ` (ahead of whatever detail the call already carries). The dispatch's wall-clock time crossed a session handoff, so the raw difference would count the handoff's idle gap as if it were work, and `null` says so instead of reporting a misleading number.
