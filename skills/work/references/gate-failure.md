...
2. Re-dispatch the **same** task at most **once**. Track infrastructure re-dispatches per task — this cap is separate from the test-failure retry cap (step 5.5) and the review-cycle cap (step 5.7). The re-dispatch keeps the first dispatch's telemetry id: close the first attempt (`record_dispatch.py end <id> --outcome lost`, or `timeout`), re-dispatch the same prompt file, and close the same id again when it returns (`references/subagent-dispatch.md` § Dispatch telemetry). No `start` call: the prompt was measured once.
