...
- Unattended sessions (`CLAUDE_UNATTENDED=1`): never call `AskUserQuestion`; take the documented default, log it as `defaulted:<decision>` in the run report, and follow `${CLAUDE_PLUGIN_ROOT}/skills/run-autopilot/references/unattended-contract.md`.
