...
# 800-line cap. Duplicates (rather than sources) the same stub-`codex`-binary
# setup and assert helpers as its sibling, so each file stays a standalone,
# runnable unit. Stubs the `codex` binary on PATH and asserts on its
# OBSERVABLE argv/stdin, never on codex-run.sh's internals.
set -u
...
run_codex_run --resume-thread "$RESUME_UUID" -o "$RESUME_OUTFILE" "$RESUME_PROMPT" \
...
run_codex_run --resume-thread "$RESUME_ID_FILE" -o "$RESUME_FILE_OUTFILE" "analyze the resume-file case" \
    > /dev/null 2>/dev/null < /dev/null
...
run_codex_run --resume-thread "" -o "$EMPTYID_OUTFILE" "analyze the empty-id case" \
...
run_codex_run --resume-thread "$EMPTY_RESUME_FILE" -o "$EMPTYFILE_OUTFILE" "analyze the empty-file case" \
...
run_codex_run -d "$ADDDIR_DIR" --resume-thread "$RESUME_UUID" -o "$ADDDIR_OUTFILE" "analyze the add-dir case" \
...
STUB_ALL_ARGV_FILE="$STUB_ALL_ARGV_FILE" STUB_INVOKE_COUNT_FILE="$STUB_INVOKE_COUNT_FILE" \
    ...
    run_codex_run --resume-thread "$RESUME_UUID" -o "$FALLBACK_OUTFILE" "analyze the fallback case" \
...
STUB_FAIL_FRESH="$FRESH_FAIL_EXIT" \
    run_codex_run --emit-thread-id "$FRESHFAIL_THREAD_ID_FILE" -o "$FRESHFAIL_OUTFILE" "analyze the fresh-fail case" \
...
run_codex_run --resume-thread "$RESUME_UUID" "analyze without -o" \
...
STUB_SUPPRESS_THREAD_STARTED=1 \
    run_codex_run --resume-thread "$RESUME_UUID" --emit-thread-id "$RESUME_NOSTART_THREAD_ID_FILE" \
...
STUB_FAIL_RESUME=5 \
    run_codex_run --resume-thread "$RESUME_UUID" --emit-thread-id "$RESUME_EMIT_FAIL_THREAD_ID_FILE" \
...
run_codex_run --resume-thread "$RESUME_UUID" -o "$RESUME_STDIN_OUTFILE" "$RESUME_STDIN_PROMPT" \
...
if child_stdin_is_prompt "$RESUME_STDIN_PROMPT"; then
