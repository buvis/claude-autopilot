...
After both sources are merged into `rework_task_ids`, update state (the sync hook maintains the task counts). **Loop mode (`$_AUTOPILOT_LOOP` set): hand off here, before any fix runs (PRD 00208).** This session already holds the review consolidation and the rework design's three dispatch packages (measured 2026-09-21: 464K at the first fix task's start, then a hard-cap rotation mid-task), so the fixes run in a fresh session: write the contract card naming the queued rework ids and the rework design doc path, then follow the **Session handoff procedure** (core `SKILL.md`: brief, `leave` row with `--site review --edge leave --phase review`) with `state.phase` and `state.next_phase` left at `review` and `state.cycle` unchanged - no `phase-done`, the cycle is not over - print `── AUTOPILOT ── PRD: {prd-name} ── rework designed, handing off {n} task(s) ──` and END TURN. The next session lands in Phase 4's skip ("Skip Phases 4 and 5 and resume at Phase 6 Dispatch rework": the cycle's review file is on disk and `rework_task_ids` names an unfinished task) and invokes `/autopilot:work` from there, on the queued tasks' own tier (`cli/routing.rework_resume`, PRD 00207). **Interactive (no `$_AUTOPILOT_LOOP`):** invoke `/autopilot:work` now — it reads `state.rework_task_ids` and enters **rework mode** (see `work/SKILL.md` "Rework-mode task filter"), processing only the listed IDs at the tier each task carries in `state.tasks[i].model`; non-listed completed tasks are skipped.
