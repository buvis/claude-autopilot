...
**Verify escapes.** The sweep's work pass runs step 7, so it also runs this cycle's queued checks — and Phase 5 never reopens after convergence, so a non-zero `verify_check:` there has no next cycle to reach. Record each one via `autopilot defer --prd <filename> --batch <batch_id> --json '{"type": "verify-escape", "command": ..., "exit": ...}'` and finalize. Without this the failed check lands in a file nothing reads and the PRD finalizes green over it.
