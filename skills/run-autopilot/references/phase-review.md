...
After both sources are merged into `rework_task_ids`, update state (the sync hook maintains the task counts). **Loop mode (`$_AUTOPILOT_LOOP` set), in a session that ran Phases 4-5 of this cycle (it did not enter through the Phase 4 skip): hand off here, before any fix runs (PRD 00208).** This holds whether the batch has `[D{cycle}]` tasks created above, only review-flagged `[C{cycle}]` tasks re-queued in Phase 5 step 4, or both: the test is how the session entered, not what it created. This session already holds the review consolidation and the rework design's three dispatch packages (measured 2026-09-21: 464K at the first fix task's start, then a hard-cap rotation mid-task), so the fixes run in a fresh session. In this order: write the contract card naming the queued rework ids and the rework design doc path; write the brief; write the `leave` row (`--site review --edge leave --phase review`); print the banner `── AUTOPILOT ── PRD: {prd-name} ── rework designed, handing off {n} task(s) ──`; END TURN. That is steps 2 and 3 of the **Session handoff procedure** (core `SKILL.md`) only: step 1 of that procedure (`phase-done`) is skipped on purpose, because the cycle is not over - `state.phase` and `state.next_phase` stay `review` and `state.cycle` is unchanged. The next session lands in Phase 4's skip ("Skip Phases 4 and 5 and resume at Phase 6 Dispatch rework": the cycle's review file is on disk and `rework_task_ids` names an unfinished task) on the queued tasks' own tier (`cli/routing.rework_resume`, PRD 00207). **A session that entered this section through that Phase 4 skip (the rework was already queued when it started) invokes `/autopilot:work` now and never hands off again: the hand-off belongs to the session that ran Phases 4-5 only.** **Interactive (no `$_AUTOPILOT_LOOP`):** invoke `/autopilot:work` now — it reads `state.rework_task_ids` and enters **rework mode** (see `work/SKILL.md` "Rework-mode task filter"), processing only the listed IDs at the tier each task carries in `state.tasks[i].model`; non-listed completed tasks are skipped.
