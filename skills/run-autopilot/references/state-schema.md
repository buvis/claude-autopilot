...
| `tasks[].tier_reason` | string? | Why the planner assigned the task its tier **at plan time**. Legal values: `"test_port"` (every path in the task's slice is a test path, pinned to `sonnet`), `"packaging"` (every path is a test or packaging path, pinned to `sonnet`), `"contract"` (the task itself edits a public contract, so `opus`), `"algorithmic_risk"` (the task's own Details name a new algorithm, shared mutable state, or a migration, so `opus`), `"mechanical"` (the haiku row), `"default"` (the `sonnet` fallback), and `"floor"` (the PRD frontmatter `default_model` floor raised the classified tier). Written once by `/autopilot:plan-tasks` step 4.7 from `classify_tier.py`'s output, taking the value the classifier printed; persisted directly in `state.tasks[]` and **never rewritten afterwards**. It therefore explains the tier the planner chose, not necessarily the tier `tasks[].model` carries now: an escalation rewrites `model` without touching this field — Phase 6's review-flag ladder (`references/phase-review.md`, `escalation_reason: "review_flag"`) and `/autopilot:work`'s in-loop `gate_failure` ladder both do. So a task reading `model: "opus"` with `tier_reason: "mechanical"` is an escalated task, not a corrupt record; `attempts[].escalated_from` and `attempts[].escalation_reason` carry that history. No reason maps to `fable`: the classifier only ever assigns `haiku`, `sonnet`, or `opus`, and that rung stays human-gated (see the `tasks[].model` row). Absent on plans created before PRD 00160. |
