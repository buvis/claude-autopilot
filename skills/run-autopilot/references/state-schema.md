...
| `tasks[].tier_reason` | string? | Why `tasks[].model` carries the tier it does. Legal values: `"test_port"` (every path in the task's slice is a test path, pinned to `sonnet`), `"packaging"` (every path is a test or packaging path, pinned to `sonnet`), `"contract"` (the task itself edits a public contract, so `opus`), `"algorithmic_risk"` (the task's own Details name a new algorithm, shared mutable state, or a migration, so `opus`), `"mechanical"` (the haiku row), `"default"` (the `sonnet` fallback), and `"floor"` (the PRD frontmatter `default_model` floor raised the classified tier). Written by `/autopilot:plan-tasks` step 4.7 from `classify_tier.py`'s output, taking the value the classifier printed; persisted directly in `state.tasks[]`. No reason maps to `fable`: the classifier only ever assigns `haiku`, `sonnet`, or `opus`, and that rung stays human-gated (see the `tasks[].model` row). Absent on plans created before PRD 00160. |
