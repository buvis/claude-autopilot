...
#
# Run: bash <this-skill>/scripts/test_autoclaude_fable_subcommands.sh
set -u

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# The skill's own root, derived rather than looked up, so the suite runs the
# same from the plugin install or a checkout. Deliberately NOT in the `unset`
# list further down: the sandbox builder still needs it after globals are cleared.
...
# NOTE: this `~` is the SANDBOX's HOME, not yours. It resolves through the
# symlink make_sandbox lays at $SBOX/.home/.claude/skills/run-autopilot, so it
# must stay a tilde path even though the skill now ships from the plugin.
