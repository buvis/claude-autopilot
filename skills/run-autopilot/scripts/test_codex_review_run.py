...
class ExitContractTests(unittest.TestCase):
    ...
    def test_quota_mentioning_finding_completes_clean_and_writes_output(
        ...
        with tempfile.TemporaryDirectory() as work:
            ...
            self.assertTrue(out.exists())
            self.assertIn(
                "the quota check in billing.py is unbounded",
                out.read_text(),
            )
...
class ExitFourOwnershipTests(unittest.TestCase):
    ...
    # Sibling skill inside this pack, located from this file rather than ~/.claude.
    pack_root = Path(__file__).resolve().parents[3]
    RUN_SH = pack_root / "skills/use-codex/scripts/codex-run.sh"
    CONTRACT = pack_root / "skills/use-codex/references/dispatch-contract.md"
