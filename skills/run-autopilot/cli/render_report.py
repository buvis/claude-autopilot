...
def _first_text(d: dict, keys: tuple[str, ...]) -> str | None:
    """The first alias in `keys` carrying text: a non-empty `str`. An empty
    string and a non-string alike are skipped and the chain keeps walking;
    None when no alias carries text."""
    for key in keys:
        value = d.get(key)
...
def _autonomous_row(d: dict) -> list:
    return [
        ...
        _first_text(
            d,
            ("issue", "question", "finding", "decision", "supersedes", "detail"),
        ),
        d.get("severity"),
        _first_text(d, ("action", "disposition")),
        _first_text(d, ("reason", "resolution", "rationale", "assumption")),
    ]
...
def is_autonomous_row(entry) -> bool:
    ...
    Made) and whose Issue cell holds text, i.e. the first of issue / question
    / finding / decision / supersedes / detail holding a non-empty string.
    Text in any other cell earns no row, because the row it would draw shows
    a blank Issue. `statectl.complete-prd` counts with this same predicate so
    the write side and the render side agree."""
    return (
