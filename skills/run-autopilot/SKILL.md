...
- Files read from other skill dirs:
  `${CLAUDE_PLUGIN_ROOT}/skills/review-work-completion/scripts/await_reviewer_outputs.py`
  (Watcher keep-alive)
...
Review sessions run on opus at xhigh in cycle 1 and at high on cycle 2 and later; _AUTOPILOT_EFFORT_REVIEW forces one effort for every cycle and _AUTOPILOT_EFFORT_REVIEW_RERUN sets the rerun value. A rework resume (the cycle's review file exists and `rework_task_ids` names an unfinished task) runs on the queued tasks' own tier instead, sonnet unless one is opus or fable (`references/model-ladder.md` § Review sessions, PRD 00207).

## Entry Points

- `/autopilot:run-autopilot` — auto-select PRD (wip first, then backlog), run full cycle
- `/autopilot:run-autopilot <prd-filename>` — full cycle with specific PRD
- `/autopilot:run-autopilot status` — print current dashboard, no action
- `/autopilot:run-autopilot review-batch` — interactively review a finished batch's deferred items
...
**`statectl` is the sole writer of `state.json`.** Apply every field mutation with `python3 ${CLAUDE_PLUGIN_ROOT}/skills/run-autopilot/scripts/statectl.py <state.json> set|append|del <json-path> <json-value>` (read one field with `get`); it takes an advisory lock, backs up to `<state.json>.bak`, changes only the named path while preserving all siblings, and replaces the file atomically — so it cannot leave a half-written or unparseable state, and it never trips the "not read yet" / "modified since read" errors the editing tools hit. Since PRD 00089 it is a shim over `cli/state.transaction`, so every write is schema-validated too: a value that is malformed **for the field it targets** is rejected with exit 1 and the file left byte-unchanged (an unrelated pre-existing odd field blocks nothing). **Phase transitions are not field mutations** — use `autopilot phase-done --outcome <outcome>` (§ Session handoff procedure), never a hand-written `set phase` / `set next_phase` pair, so a transition cannot land without the effects it mandates. Resolve `<state.json>` by walking up from the resolved physical cwd to the autopilot dir (the `_walk_up.py` pattern in `/autopilot:work` step 2). The terse `set X` / `write field Y` / per-PRD-reset instructions throughout these skills and the gate files name the fields to change; each is applied with `statectl`, never by hand-editing the file.
...
- **Durable** (never delete): `dev/local/prds/done/` and its PRDs, `dev/local/reviews/` (per-cycle review files, blind/doubt reviews, and the `<prd-base>-audit.md` audit renders), `dev/local/designs/` (per-PRD design docs), `dev/local/autopilot/ledger/loop-metrics.jsonl` (the GC-exempt mirror, the durable copy to read), `dev/local/autopilot/ledger/attempts.jsonl` (per-attempt rows appended at every PRD close, `references/state-schema.md` § Attempt ledger), `dev/local/autopilot/ledger/custody.jsonl` (the custody journal, the push guard's restore source when purge-devlocal trashes the `critical-on-master` marker, `references/state-schema.md` § Custody journal), `dev/local/autopilot/ledger/dispatch-metrics.jsonl` (per-dispatch timing rows and session-handoff edges, `work/references/subagent-dispatch.md` § Dispatch telemetry), and `dev/local/meta/project-capsule.md`.
- **Not durable, despite living under `dev/local/autopilot/`**: `purge-devlocal` exempts only `autopilot/ledger/**`. `dev/local/autopilot/loop-metrics.jsonl`, `dev/local/autopilot/dispatch-metrics.jsonl`, `dev/local/autopilot/deferred/` (the `{batch_id}-deferred.json` records), and `dev/local/autopilot/reports/` (batch reports) all fall through to the `stale-autopilot` rule and get trashed once their mtime passes the autopilot age threshold (14 days by default). An idle repo loses them past that window; only the `ledger/` mirror above survives. Known limitation: promotion signal 3b reads `dev/local/autopilot/deferred/*-deferred.json` directly and has no `ledger/` mirror behind it, so it carries this same 14-day exposure.
- **Disposable** (safe to delete by name): `dev/local/tmp/`, `dev/local/autopilot/last-session.log`, `dev/local/autopilot/wrapper.log`, `dev/local/autopilot/pause-requested`, `dev/local/autopilot/paused-by-operator`, `dev/local/autopilot/session-brief.md` (re-rendered at every handoff), `dev/local/autopilot/.session-left` (overwritten at every loop-mode handoff), `dev/local/autopilot/lanes/` (one marker per live CLI reviewer lane, removed when the lane exits), `dev/local/autopilot/state.json` (at batch end only — in loop mode the wrapper archives it to `reports/{batch_id}-state-final.json`; see `references/phase-done.md`), `dev/local/autopilot/wave.json` and `dev/local/autopilot/wave-slots/` (a wave's control file and its per-wave review-slot directory, both disposable at wave end — neither is a durable PRD-lifecycle artifact; `references/waves.md`), and `dev/local/autopilot/replan-context.md`.
...
When `/autopilot:run-autopilot` is invoked and `dev/local/autopilot/state.json` exists with `batch.completed_prds`, this is a continuation after a session restart. Preserve `batch.completed_prds` (including `batch.id`) and proceed to the build gate's Phase 0 to pick the next PRD.
...
- **Pause**: `touch dev/local/autopilot/pause-requested` — the wrapper consumes the marker at the next session boundary, notifies "paused by operator", and stops the loop with state intact. It leaves `paused-by-operator` behind so tracon shows `⏸ paused` rather than `⚠ orphaned`; plain `autoclaude` resumes (nothing to clear, no decision to make). The marker may carry a JSON reason, `{"reason": "..."}`, which the stop line and the notification quote; a session that stands down for a peer writes one with a `condition` beside it, after asking the peer first (§ Session Loop, stand-down procedure).
- **Take over**: with the loop stopped (pause marker, or Ctrl-C on the wrapper), run `/autopilot:run-autopilot` in a normal interactive session — resume-by-artifact reads the same `state.json`, and full interactive semantics (questions, PAUSEs) apply because `$_AUTOPILOT_LOOP` is unset. Restart `autoclaude` afterwards to go unattended again.
...
- **Run a wave (parallel lanes)**: `autopilot wave plan`, then `autopilot wave launch`, `autopilot wave status` and `autopilot wave abort` — one detached loop per lane worktree, each over its own slice of the backlog. Full operator surface, including what is deferred: `references/waves.md`.
...
A running headless turn is never interrupted except by the wrapper's wall-clock cap (`_AUTOPILOT_SESSION_MAX`, default 7200s); review-phase launches use `_AUTOPILOT_SESSION_MAX_REVIEW` (default 10800s). Fifteen minutes before the cap (`_AUTOPILOT_SESSION_WARN`, seconds, `0` disables) the wrapper writes the same `.handoff-requested` marker the context-cap hook writes, so a session that reaches a task boundary inside that window hands off cleanly instead of being killed mid-task.
...
**Stand-down procedure (a peer session owns the PRD).** A headless session never works a PRD another session is already working: two writers on one `state.json` and one set of task artifacts corrupt both, and the one-loop-per-repo guard sees only wrappers, not a bare interactive session. Phase 0 asks before it pauses. A peer counts only when `ListAgents` shows a busy interactive session whose name starts with this repository's directory basename (`basename "$PWD"`, e.g. `agent-skills-`); a session named for another repository is never evidence, whatever it is busy with (the two 2026-09-14 false stand-downs in agent-skills each cited a `claude-autopilot-*` session, the other repo's own loop). When such a peer exists, send it one `SendMessage` naming the PRD and wait at most 120 s for a reply. Stand down only when the reply claims the PRD (condition `peer_claimed`), or when writer evidence is present: `git status --porcelain` lists a tracked path this session did not edit (condition `dirty_tree`), or the mtime of `state.json` or `contract-card.md`, truncated to whole seconds, is greater than the `at` of the most recent `kind: "handoff"` row with `edge: "leave"` and this PRD's `prd` in `dev/local/autopilot/dispatch-metrics.jsonl` (condition `state_after_leave`; the rows carry no batch id, so the `prd` filter is what keeps an older batch's row out; when no such `leave` row exists, skip the mtime test and let the dirty-tree test and the reply decide). That comparison holds only because every handoff site writes its `leave` row AFTER its last state and card write (§ Session handoff procedure orders step 1 before step 2; `/autopilot:work`'s task-boundary handoff must set `next_phase` before its row too): a site that touches `state.json` after its row makes every next session read its own hand-off as peer evidence. A bare recent mtime is not evidence (a reader that inspects state while the loop writes, or this batch's own per-PRD reset, satisfies it), commit trailers are not consulted (no commit here carries a session trailer), and a peer that only reads, or never answers, never pauses a batch. To stand down: do not select, move, or write anything else; write `dev/local/autopilot/pause-requested` with the Write tool as one JSON line, `{"reason": "<peer session> owns <prd> (<evidence>)", "condition": "<peer_claimed | dirty_tree | state_after_leave>"}`, and end the turn. The loop reads a marker written during the session as a stand-down (branch 5 above): it stops with state intact, burns no retry, writes no park marker, quotes the reason in its stop line and notification, and records `stood_down` and `stood_down_condition` in the session's `loop-metrics.jsonl` row; `autoclaude` alone resumes once the peer is done. Before this procedure existed (2026-09-02, agent-skills PRD 00010) four stand-downs read as deaths and parked a healthy PRD mid-task.

The model's only job at a hand-off is to write `state.next_phase` (and `phase`/`stall_reason`) accurately, print the banner, and end the turn. The model never writes any signal and never inspects the loop's decision table — writing accurate state IS the hand-off. Interactive (non-loop) semantics are identical minus the loop: the same state writes happen, and the user re-invokes `/autopilot:run-autopilot` manually.
...
2. **Write the session brief, then the `leave` handoff row, then print the site's banner** (shown at the invoking site in the gate file). The brief comes first, after the contract card (§ Contract card): `python3 ${CLAUDE_PLUGIN_ROOT}/skills/run-autopilot/scripts/statectl.py <state.json> write-brief dev/local/autopilot/session-brief.md` renders `dev/local/autopilot/session-brief.md` from the state step 1 committed (where the batch stands, the card verbatim, the files the next gate needs), so the next session orients from one page (PRD 00201); the loop's launch prompt names it whenever it exists. A failed write is one stderr line, never a phase failure: carry on to the row. The row is `python3 ${CLAUDE_PLUGIN_ROOT}/skills/work/scripts/record_dispatch.py handoff --site <build|review|done> --edge leave --phase <state.phase> --prd <state.prd>`, best-effort (`work/references/subagent-dispatch.md` § Dispatch telemetry); `state.phase` already names the next session's phase because step 1 committed it. A new site row in the table below gets its row from this step, not from a per-site sentence. Under `$_AUTOPILOT_LOOP` the row is also the session's point of no return: `hooks/note_session_leave.py` records it, and `hooks/guard_skill_after_leave.py` denies any `autopilot:*` (or `git-ferry:catchup`) `Skill` call this session makes afterwards, so a session that prints its banner and then re-invokes a gate is stopped by the harness, not only by this prose (PRD 00211).
...
| **build → done** (`references/lane-solo.md` § Close, a lane-routed PRD whose one review pass wrote its review file) | `lane_reviewed` | Convergence's own effect set: `phase`/`next_phase: "done"` and `"review"` appended once to `phases_completed`, so `review_coverage_hook.py` gates the lane's `-review-1.md` with `--require-codex-guard` exactly as a full-lane cycle's. An escalating solo build takes the **build → review** row instead. |
...
- **Autopilot (state.json world):** write the body to `dev/local/autopilot/contract-card.md` with the **Write tool**, then load it with one call:
  ```bash
  python3 ${CLAUDE_PLUGIN_ROOT}/skills/run-autopilot/scripts/statectl.py <state.json> set-contract-card dev/local/autopilot/contract-card.md
  ```
...
The session brief (`write-brief`, § Session handoff procedure step 2) is rendered right after this call so it carries the card just loaded; `/autopilot:work`'s task-boundary handoff and `/autopilot:review-work-completion`'s cycle transition write the brief the same way where they write their cards.

`reinject_contract_card.py` is a SessionStart hook **matched to `compact` only** (startup/resume/clear stay unmatched, so there is no standing token cost); after a compaction it reads the card back and re-injects it as additionalContext. `/autopilot:work` (task-boundary transitions) and `/autopilot:review-work-completion` (cycle transitions) write their own cards the same way — `contract_card` in `references/state-schema.md`.
...
**If `git rev-parse HEAD` fails at capture** — an unborn branch or a repo with no commits yet (a *detached* HEAD resolves fine and is not this case; a bare-repo-backed root like `~/.claude` must run rev-parse in the SAME git context `repo_root` uses — the `--git-dir`/`--work-tree` flags — not a bare `git rev-parse` that would fail or resolve a parent repo): there is no base commit to diff against. Do NOT leave `work_start_sha` unset (an absent value makes the review range undefined and crashes the doubt lens). Set it to the git **empty-tree sentinel** `4b825dc642cb6eb9a060e54bf8d69288fbee4904`, log a one-line warning, and note the fallback in the batch report — the review range `4b825dc…..HEAD` then spans the whole tree once `/autopilot:work` has committed, which is the correct scope for a first-commit repo.
...
| Git push fails (auth, locked signing agent, network) | Report and let the user retry. A push denied by hooks/guard_push_on_critical.py names a pending cap_critical custody: resolve it with autopilot custody resolve, never bypass the hook. | Log to `deferred_decisions[]`, leave the commits local (the user pushes manually per Phase 9), CONTINUE — a locked signing agent on an unattended host is expected (`references/design-rationale.md` § Git push failures). A guard denial is closed at the next attended Phase 0 (`references/phase-build.md` § Handle pending custody), where the operator picks revert, branch-and-revert or accept; loop mode leaves the commits local and continues without any git mutation. |
...
1. **Security-critical finding** — exposed secret or a vulnerability being shipped (including a cap_critical custody capture that fails twice) (skill PAUSE; `pause_reason` set by the Error Handling row above).
...
- `references/waves.md` — the `autopilot wave` verbs, what a lane worktree holds, and what is deferred
