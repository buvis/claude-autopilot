...
- **Disposable** (safe to delete by name): `dev/local/tmp/`, `dev/local/autopilot/last-session.log`, `dev/local/autopilot/wrapper.log`, `dev/local/autopilot/pause-requested`, `dev/local/autopilot/paused-by-operator`, `dev/local/autopilot/session-brief.md` (re-rendered at every handoff), `dev/local/autopilot/.session-left` (overwritten at every loop-mode handoff), `dev/local/autopilot/state.json` (at batch end only — in loop mode the wrapper archives it to `reports/{batch_id}-state-final.json`; see `references/phase-done.md`), and `dev/local/autopilot/replan-context.md`.
