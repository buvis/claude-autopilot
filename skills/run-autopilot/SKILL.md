...
**Stand-down procedure (a peer session owns the PRD).** A headless session never works a PRD another session is already working: two writers on one `state.json` and one set of task artifacts corrupt both, and the one-loop-per-repo guard sees only wrappers, not a bare interactive session. When Phase 0 finds evidence of a live owner — `ListAgents` shows a busy interactive peer session in this repo AND `state.json`, the contract card, or the PRD's task artifacts changed within the last 15 minutes — do not select, move, or write anything. Write `dev/local/autopilot/pause-requested` with the Write tool as one JSON line, `{"reason": "<peer session> owns <prd> (<evidence>)"}`, and end the turn. The loop reads a marker written during the session as a stand-down (branch 5 above): it stops with state intact, burns no retry, writes no park marker, and quotes the reason in its stop line and notification; `autoclaude` alone resumes once the peer is done. Before this procedure existed (2026-09-02, agent-skills PRD 00010) four stand-downs read as deaths and parked a healthy PRD mid-task.
