...
# ${CLAUDE_PLUGIN_ROOT}/skills/review-work-completion/SKILL.md:
#
...
#
# Run: bash <this-skill>/scripts/test_codex_doubt_guard.sh
set -u

# Paths are derived from this script's own location, so the suite runs the same
# from the plugin install, a git checkout, or a worktree.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SKILL_MD="$SCRIPT_DIR/../SKILL.md"
CHECK_SCRIPT="$SCRIPT_DIR/check_review_file.py"
