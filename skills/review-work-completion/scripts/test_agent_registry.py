"""Hygiene checks for the reviewer agent registry (${CLAUDE_PLUGIN_ROOT}/agents/).
...
# Derived from this file rather than from ~/.claude: the personas and the skills
# that dispatch them ship with the plugin now, and the personal copies are
# retired. parents[3] is the pack root (scripts -> skill -> skills -> pack).
_PACK_ROOT = Path(__file__).resolve().parents[3]
AGENTS_DIR = _PACK_ROOT / "agents"
_SKILL_DIR = _PACK_ROOT / "skills" / "review-work-completion"
...
def test_no_persona_prompt_text_survives_outside_the_registry() -> None:
    ...
    # The pack's own skills/ is where a leak would now live; ~/.claude/workflows
    # stays in the sweep because review-fanout.workflow.js is still hosted there.
    sweep_roots = (_PACK_ROOT / "skills", Path.home() / ".claude" / "workflows")
    signatures = {
...
def test_placeholders_are_declared_in_the_conventions(registry) -> None:
    ...
    conventions = (_SKILL_DIR / "references" / "agent-registry.md").read_text(
        encoding="utf-8",
    )
    used: set[str] = set()
    for name in ROSTER:
...
def _eve_invocation_section() -> str:
    ...
    """
    text = (_SKILL_DIR / "references" / "agent-invocation.md").read_text(
        encoding="utf-8",
    )
    match = re.search(
        r"^## Eve \(Fable 5\)$(.*?)(?=^## |\Z)",
