...
driving the exported parsing helpers live in test_guard_push_grammar.py; the
`decide` / `run` tests here work against throwaway `git init` repositories
(one seeded with a custody marker, journal row and locator, one clean) through
payload dicts. No test executes a real push: the guard only inspects command
text and custody state on disk.
...
class RegistrationTests(unittest.TestCase):
    def test_bash_matcher_registers_the_guard_once_beside_enforce_prd_location(
        ...
        self.assertEqual(
            len(bash_blocks),
            1,
            "exactly one PreToolUse block matches Bash",
        )
